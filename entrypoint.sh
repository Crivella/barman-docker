...
echo "${BARMAN_BACKUP_SCHEDULE} barman /usr/local/bin/barman cron && barman /usr/local/bin/barman backup all" >> /etc/cron.d/barman
...
prometheus-node-exporter --web.listen-address="${NODE_EXPORTER_LISTEN_ADDRESS}:${NODE_EXPORTER_LISTEN_PORT}" &
